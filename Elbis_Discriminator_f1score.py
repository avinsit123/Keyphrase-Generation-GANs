...
from Elbis_Discriminator import Discriminator
...
#    print("Test 1 Passed")
    ...
    real_rewards = D_model.calculate_context_rewards(abstract_t,kph_t,1,len_list_t)  
    fake_rewards = D_model.calculate_context_rewards(abstract_f,kph_f,0,len_list_f)  
    
    return real_rewards,fake_rewards
...
def main():
   ...
   # model.load_state_dict(torch.load("model/kp20k.ml.one2many.cat.copy.bi-directional.20190628-114655/kp20k.ml.one2many.cat.copy.bi-directional.epoch=2.batch=54573.total_batch=116000.model"))
    ...
    D_model.load_state_dict(torch.load("Discriminator_checkpts/elbis_Dis_3.pth.tar"))
