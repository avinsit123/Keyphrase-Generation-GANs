...
def generate_baseline_rewards(model,src,mlt_kph,grd_kph,total_len):
    grd_kph = remove_duplications(grd_kph)
    total_mlt_kphs = len(mlt_kph)
    total_grd_kphs = len(grd_kph)
    temp_mlt_kphs = mlt_kph
    temp_grd_kphs = grd_kph
    mlt_src = model.transform_src(src,total_mlt_kphs)  
    grd_src = model.transform_src(src,total_grd_kphs)  
    mlt_kph = model.padded_all(mlt_kph,total_mlt_kphs,model.pad_idx)
    grd_kph = model.padded_all(grd_kph,total_grd_kphs,model.pad_idx)    
    mlt_src,mlt_kph = mlt_src.long().to(devices),mlt_kph.long().to(devices)
    grd_src,grd_kph = grd_src.long().to(devices),grd_kph.long().to(devices)
    mlt_rewards = model.forward(mlt_src,mlt_kph)
    mlt_rewards = model.sigmoid(mlt_rewards) 
    grd_rewards = model.forward(grd_src,grd_kph)
    grd_rewards = model.sigmoid(grd_rewards)
    print("MLT:",mlt_rewards)
    print("GRD:",grd_rewards)
   ...
   # print(pred_str_2dlist)
    ...
    with torch.no_grad():
        ...
        greedy_str_2dlist = sample_list_to_str_2dlist(greedy_sample_list, oov_lists, opt.idx2word, opt.vocab_size,
                                                      eos_idx,delimiter_word, opt.word2idx[pykp.io.UNK_WORD], opt.replace_unk,
                                                      src_str_list, opt.separate_present_absent, pykp.io.PEOS_WORD)
   # print(greedy_str_2dlist)
   # sys.exit()
    ...
    batch_mine = 0
    all_rewards = 0
    for idx, (src_list, pred_str_list,target_str_list,greedy_str_list) in enumerate(zip(src, pred_str_2dlist,target_str_2dlist,greedy_str_2dlist)):

        print("Abstract ",Doit(list(src_list.cpu().numpy()),opt.idx2word))
        print("Multinomial",DIT(pred_str_list,opt.idx2word))
        print("Greedy",DIT(remove_duplications(greedy_str_list),opt.idx2word))
        print("True",DIT(target_str_list,opt.idx2word))
        continue

        batch_mine+=1
        if (len(target_str_list)==0 or len(pred_str_list)==0):
            continue
        greedy_str_list = remove_duplications(greedy_str_list)
        generate_baseline_rewards(D_model,src_list,pred_str_list,greedy_str_list,total_len)
...
def main():
    ...
    model.load_state_dict(torch.load("RL_Checkpoints/Checkpoint(2_Disc_combined)0.pth.tar"))
    ...
    D_model.load_state_dict(torch.load("Discriminator_checkpts/D_model_combined0.pth.tar"))
