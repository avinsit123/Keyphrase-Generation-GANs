...
from evaluate1 import evaluate_loss
import time
import math
import logging
import torch
    ...
    for epoch in range(opt.start_epoch, opt.epochs+1):
        ...
        for batch_i, batch in enumerate(train_data_loader):
            ...
            if opt.train_ml:
                ...
                # Brief report
              #  print("The Batch LOss is",batch_loss_stat.xent())
                '''
...
def train_one_batch(batch, model, optimizer, opt, batch_i, source_representation_queue=None):
    if not opt.one2many:  # load one2one data
     ...
     ## sec_str_list 
     ## trg_str_2dlist
    trg, trg_lens, trg_mask, src, src_lens, src_mask, trg_oov, src_oov     = src, src_lens, src_mask, trg, trg_lens, trg_mask, src_oov, trg_oov
#    print(src_lens)
#    print(sorted(list(zip(src_lens,src.numpy())),reverse=True))
#    src = [x for _,x in sorted(list(zip(src_lens,src)),reverse=True)]
#    src_lens = sorted(src_lens,reverse=True)
#    
#    print("New:",src)
    num_trgs = list(torch.ones(opt.batch_size).long().numpy())
