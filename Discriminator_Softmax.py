...
class Discriminator(S_RNN):
    def __init__(self,vocab_size,embedding_dim,hidden_dim,n_layers,pad_idx,bidirectional=False):
        ...
        self.RNN1 = S_RNN(embedding_dim,hidden_dim,n_layers,bidirectional=False)  ### Abstract RNN
        self.RNN2 = S_RNN(embedding_dim,hidden_dim,n_layers,bidirectional=False)  ### Summary RNN
    ...
    def forward(self,src,kph):
        ...
        #print("The size of the source after the embeddings is",src.size())
        #print("The size of the target after the embeddings is",kph.size())
        ...
        #print(abstract_d.size(),keyphrase_d.size())
        since = time()
        concat_out = Stacker(abstract_d,keyphrase_d)
        #print("The time taken is",time()-since)
        since = time()
        concat_out = concat_out.to("cuda:2")
        output = self.Linear1(concat_out)
        output = self.tanh(output)
        output = self.Linear2(output)
        output = output.squeeze(2)
        output = self.Softmax(output)
        #print("The time taken 2 is",time()-since)
        modified_context = matmul(abstract_d,output)
        sumnow = torch.sum(modified_context,dim=1)
        #print("safds",sumnow.size(),keyphrase_d.size())
        concat_hidden = torch.cat((sumnow,keyphrase_d.squeeze(2)),dim=1)
        hidden_out = self.Linear(concat_hidden)
        return hidden_out.squeeze(1)
  
    def train_one_abstract(self,src,kph,target_type):
        #phrase_reward = np.zeros((batch_size, max_num_phrases))
        #phrase_reward = 
        print("is:",src)
        print("iss:",kph)
...
#            results = torch.ones(total_len)*0.9
            results = torch.from_numpy(np.random.uniform(0.7, 1, size=(total_len))).float()
            #print(output.size(),results.size())
            if torch.cuda.is_available():
                results = results.to("cuda:2")
        else:
#            results = torch.zeros(total_len)
            results = torch.from_numpy(np.random.uniform(0, 0.3, size=(total_len))).float()
            if torch.cuda.is_available():
                results = results.to("cuda:2")
        #print(output)
        criterion = nn.BCELoss()
        avg_outputs = torch.mean(nn.Sigmoid()(output))
...
def Stacker(abstract,kph):
    temp = torch.randn(abstract.size(0),abstract.size(1),2*abstract.size(2))
    kph = kph.squeeze(2)
    #print(kph.size())
    for i in range(abstract.size(0)):
        for j in range(abstract.size(1)):
            #print(temp[i,j,:].size(),abstract[i,j,:],kph[i,:].size())
            #print(abstract[i,j,:].size())
            #print(kph[i,:].squeeze(1).size())
            temp[i,j,:] = torch.cat((abstract[i,j,:],kph[i,:].squeeze(0)))
    return temp        
# abstract,kph = torch.randn(5,302,15),torch.randn(5,15,1) 