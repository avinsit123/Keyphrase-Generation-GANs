...
#    print("Test 1 Passed")
  ...
  #  print(h_kph_t_size,h_kph_f_size)  
   ...
   # print(posit,negit)
    len_abstract = abstract_t.size(1)
    true_r = D_model.calc_keyphrase_score(posit,len_abstract,len_list_t)
    fake_r = D_model.calc_keyphrase_score(negit,len_abstract,len_list_f)
    return true_r,fake_r
...
def main():
   ...
   # model.load_state_dict(torch.load("model/kp20k.ml.one2many.cat.copy.bi-directional.20190628-114655/kp20k.ml.one2many.cat.copy.bi-directional.epoch=2.batch=54573.total_batch=116000.model"))
    model.load_state_dict(torch.load("model/kp20k.ml.one2many.cat.copy.bi-directional.20190715-132016/kp20k.ml.one2many.cat.copy.bi-directional.epoch=3.batch=26098.total_batch=108000.model",map_location=devices))
