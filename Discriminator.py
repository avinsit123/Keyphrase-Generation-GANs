...
class Discriminator(S_RNN):
    ...
    def forward(self,src,kph):
        ...
        abstract_v = torch.sum(abstract_d**2,dim=2)
        kph_v = torch.sum(keyphrase_d**2,dim=1)
        mul_v = abstract_v * kph_v 
        cosine_results = torch.bmm(abstract_d,keyphrase_d).squeeze(2)  ## Size(n_kphs,max_len)
        cosine_results = cosine_results/mul_v
        
        modified_context = matmul(abstract_d,cosine_results)
        sumnow = torch.sum(modified_context,dim=1)
        #print(sumnow.size(),keyphrase_d.size())
